...
__global__ void particle_like(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float d=x[i]-g[i]; y[i]=expf(-0.5f*d*d);
  }
}
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 2.0f * hs(i, 123); hg[i] = 2.0f * hs(i, 321); }
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; particle_like<<<grid,tpb>>>(dx,dg,dy,n);
