...
__global__ void bnbAdamWDecayStep(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float m=0.9f*x[i]+0.1f*g[i]; float v=0.99f*x[i]*x[i]+0.01f*g[i]*g[i]; y[i]=x[i]-0.001f*m/(sqrtf(v)+1.0e-6f)-0.0001f*x[i];
  }
}
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 1.0f * hs(i, 123); hg[i] = 1.0f * hs(i, 321); }
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; bnbAdamWDecayStep<<<grid,tpb>>>(dx,dg,dy,n);
