...
//
// The __global__ kernel below (testKernel) is reproduced, unmodified, from:
//   NVIDIA/cuda-samples,
//   cpp/0_Introduction/simpleAtomicIntrinsics/simpleAtomicIntrinsics_kernel.cuh
//   Copyright (c) 2022, NVIDIA CORPORATION. All rights reserved.
//   SPDX-License-Identifier: BSD-3-Clause
//
// Deterministic setup: numThreads = 256, numBlocks = 64 (len = 16384 threads
// total). Initial output: all 0, except slot 8 (AND) and slot 10 (XOR) set to
// 0xff so those two tests start from a non-trivial value.
//
// Output: argv[1] (default output/cuda_output.txt), 11 lines, the final integer
// value of each output slot. Correctness is checked out-of-process by
// tests/verify.py (CPU reference: 9 order-independent slots matched exactly;
// the order-dependent atomicExch / atomicCAS slots range-checked to a valid
// thread id in [0, len)).
