...
__global__ void bnbMomentUpdate2(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    y[i] = 0.9f * x[i] + 0.1f * g[i];
  }
}
int main(int argc, char **argv) {
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; bnbMomentUpdate2<<<grid,tpb>>>(dx,dg,dy,n);
