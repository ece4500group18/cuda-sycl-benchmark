...
__global__ void vllmGeluTanhMul2(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float v=x[i]; float u=0.7978845608f*(v+0.044715f*v*v*v); float gelu=0.5f*v*(1.0f+tanhf(u)); y[i]=gelu*g[i];
  }
}
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 3.0f * hs(i, 123); hg[i] = 2.0f * hs(i, 321); }
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; vllmGeluTanhMul2<<<grid,tpb>>>(dx,dg,dy,n);
