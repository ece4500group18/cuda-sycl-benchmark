...
__global__ void ggmlEluKernel(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float v=x[i]; y[i]=(v>0.0f)?v:(expf(v)-1.0f);
  }
}
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 3.0f * hs(i, 123); hg[i] = 1.0f * hs(i, 321); }
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; ggmlEluKernel<<<grid,tpb>>>(dx,dg,dy,n);
