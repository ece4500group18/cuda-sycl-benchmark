...
__global__ void vllmSoftcapLogits(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    const float s = 30.0f; y[i] = s * tanhf(x[i] / s);
  }
}
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 10.0f * hs(i, 123); hg[i] = 1.0f * hs(i, 321); }
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; vllmSoftcapLogits<<<grid,tpb>>>(dx,dg,dy,n);
