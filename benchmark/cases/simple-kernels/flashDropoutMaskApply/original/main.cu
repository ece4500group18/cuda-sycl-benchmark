...
__global__ void dropout_apply(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float keep = h01((unsigned)i, 999u) > 0.1f ? 1.0f : 0.0f; y[i] = x[i] * keep / 0.9f;
  }
}
...
int main(int argc, char **argv) {
  ...
  dropout_apply<<<blocks,tpb>>>(dx,dg,dy,n);
