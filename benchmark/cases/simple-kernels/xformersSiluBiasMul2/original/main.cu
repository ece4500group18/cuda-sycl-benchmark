...
__global__ void xformersSiluBiasMul2(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float v=x[i]+0.125f; y[i]=(v/(1.0f+expf(-v)))*g[i];
  }
}
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 4.0f * hs(i, 123); hg[i] = 1.5f * hs(i, 321); }
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; xformersSiluBiasMul2<<<grid,tpb>>>(dx,dg,dy,n);
