...
__global__ void gelu_mul(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float v=x[i]; float gelu=0.5f*v*(1.0f+erff(v*0.70710678118f)); y[i]=gelu*g[i];
  }
}
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 5.0f * hs(i, 123); hg[i] = 2.0f * hs(i, 321); }
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; gelu_mul<<<grid,tpb>>>(dx,dg,dy,n);
