...
__global__ void bias_relu_epi(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float v=x[i]+0.125f*g[i]; y[i]=v>0.0f?v:0.0f;
  }
}
int main(int argc, char **argv) {
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; bias_relu_epi<<<grid,tpb>>>(dx,dg,dy,n);
