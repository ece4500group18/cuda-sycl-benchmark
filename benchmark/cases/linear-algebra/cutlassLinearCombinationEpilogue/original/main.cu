...
__global__ void cutlassLinearCombinationEpilogue(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    y[i] = 1.25f * x[i] + 0.5f * g[i] + 0.125f;
  }
}
int main(int argc, char **argv) {
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; cutlassLinearCombinationEpilogue<<<grid,tpb>>>(dx,dg,dy,n);
