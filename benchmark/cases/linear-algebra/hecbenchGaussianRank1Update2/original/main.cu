...
__global__ void hecbenchGaussianRank1Update2(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float pivot=1.25f; y[i]=x[i]-g[i]*0.125f/pivot;
  }
}
int main(int argc, char **argv) {
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; hecbenchGaussianRank1Update2<<<grid,tpb>>>(dx,dg,dy,n);
