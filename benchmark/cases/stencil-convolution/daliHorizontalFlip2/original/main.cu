...
__global__ void imgop(const float*x,float*y,int N,int H,int W,int C){int idx=blockIdx.x*blockDim.x+threadIdx.x,total=N*H*W*C;if(idx<total){int c=idx%C;int tmp=idx/C;int w=tmp%W;tmp/=W;int h=tmp%H;int n=tmp/H;int sw=W-1-w;y[idx]=x[((n*H+h)*W+sw)*C+c];}}
