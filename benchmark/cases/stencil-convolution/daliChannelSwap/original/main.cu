...
__global__ void image_kernel(const float*x,float*y,int N,int H,int W,int C){int idx=blockIdx.x*blockDim.x+threadIdx.x,total=N*H*W*C;if(idx<total){int c=idx%3; int base=idx-c; y[idx]=x[base+(2-c)];}}
