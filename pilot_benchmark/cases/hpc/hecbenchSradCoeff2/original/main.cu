...
__global__ void hecbenchSradCoeff2(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float g2=x[i]*x[i]+0.25f*g[i]*g[i]; y[i]=1.0f/(1.0f+sqrtf(g2+1.0e-6f));
  }
}
int main(int argc, char **argv) {
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; hecbenchSradCoeff2<<<grid,tpb>>>(dx,dg,dy,n);
