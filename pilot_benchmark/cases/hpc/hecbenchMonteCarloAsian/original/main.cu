...
__global__ void asian_payoff(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float s=100.0f*expf(0.02f*x[i]); float avg=0.5f*(s + 100.0f*(1.0f+0.01f*g[i])); y[i]=fmaxf(avg-100.0f,0.0f);
  }
}
int main(int argc, char **argv) {
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; asian_payoff<<<grid,tpb>>>(dx,dg,dy,n);
