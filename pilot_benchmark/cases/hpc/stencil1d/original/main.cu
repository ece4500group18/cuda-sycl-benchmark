// stencil1d: 3-point stencil out[i] = 0.25*in[i-1] + 0.5*in[i] + 0.25*in[i+1].
// Edge-clamped boundaries. in[i] = h01(i, 123), n = 100000.
#include <cstdio>
#include <cstdlib>
...
__global__ void stencil1d(const float *in, float *out, int n) {
  ...
  if (i < n)
    out[i] = 0.25f * in[cl(i-1,n-1)] + 0.5f * in[i] + 0.25f * in[cl(i+1,n-1)];
}
