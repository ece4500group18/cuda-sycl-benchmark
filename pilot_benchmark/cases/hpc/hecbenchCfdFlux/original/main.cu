...
__global__ void cfd_flux(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float rho=fabsf(x[i])+1.0f; float mom=g[i]; float e=0.5f*rho+0.25f*mom*mom; y[i]=mom+0.1f*(e/rho)+sqrtf(rho);
  }
}
int main(int argc, char **argv) {
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; cfd_flux<<<grid,tpb>>>(dx,dg,dy,n);
