...
__global__ void silu_mul_kernel(const float *x, const float *g, float *y, int n) {
...
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 6.0f * hs(i, 123); hg[i] = 2.0f * hs(i, 321); }
  ...
  silu_mul_kernel<<<blocks,tpb>>>(dx,dg,dy,n);
