...
__global__ void cutlassBiasGeluEpilogue2(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float v=x[i]+0.25f*g[i]; y[i]=0.5f*v*(1.0f+tanhf(0.7978845608f*(v+0.044715f*v*v*v)));
  }
}
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 2.0f * hs(i, 123); hg[i] = 1.0f * hs(i, 321); }
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; cutlassBiasGeluEpilogue2<<<grid,tpb>>>(dx,dg,dy,n);
