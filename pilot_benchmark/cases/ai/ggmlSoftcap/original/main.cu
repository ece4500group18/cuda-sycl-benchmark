...
__global__ void softcap_kernel(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float cap = 30.0f; y[i] = cap * tanhf(x[i] / cap);
  }
}
...
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 40.0f * hs(i, 123); hg[i] = 1.0f * hs(i, 321); }
  ...
  softcap_kernel<<<blocks,tpb>>>(dx,dg,dy,n);
