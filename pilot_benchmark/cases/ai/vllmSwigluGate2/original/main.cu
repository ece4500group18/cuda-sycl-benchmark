...
__global__ void vllmSwigluGate2(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float v=x[i]; y[i] = (v / (1.0f + expf(-v))) * g[i];
  }
}
int main(int argc, char **argv) {
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; vllmSwigluGate2<<<grid,tpb>>>(dx,dg,dy,n);
