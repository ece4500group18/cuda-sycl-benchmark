...
__global__ void transpose_hwc_chw(const float *in, float *out, int batch, int h, int w, int c) {
  ...
  int ch = (idx / (w * h)) % c;
  int n = idx / (w * h * c);
  out[idx] = in[((n * h + y) * w + x) * c + ch];
}

int main(int argc, char **argv) {
  const int batch = 2, h = 17, w = 19, c = 3, total = batch * h * w * c;
  ...
  int tpb = 128;
  transpose_hwc_chw<<<(total + tpb - 1) / tpb, tpb>>>(din, dout, batch, h, w, c);
