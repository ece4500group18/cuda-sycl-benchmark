// rmsnorm: per-row RMSNorm with gamma=1, eps=1e-6.
//   y = x / sqrt(mean(x^2) + eps).  rows=512, cols=1024. x[idx]=h01(idx,123)
#include <cstdio>
#include <cstdlib>
...
__global__ void rmsnorm(const float *x, float *y, int rows, int cols) {
  int r = blockIdx.x * blockDim.x + threadIdx.x;
  if (r < rows) {
    const float *xr = x + (size_t)r * cols;
    float *yr = y + (size_t)r * cols;
    float ms = 0.0f;
    for (int c = 0; c < cols; ++c) ms += xr[c] * xr[c];
    ms /= cols;
    float inv = rsqrtf(ms + 1e-6f);
    for (int c = 0; c < cols; ++c) yr[c] = xr[c] * inv;
  }
}
...
int main(int argc, char **argv) {
  ...
  rmsnorm<<<blocks,tpb>>>(dx,dy,rows,cols);
