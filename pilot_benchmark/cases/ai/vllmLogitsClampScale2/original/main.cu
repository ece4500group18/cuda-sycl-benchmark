...
__global__ void vllmLogitsClampScale2(const float *x, const float *g, float *y, int n) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    float v=fminf(6.0f,fmaxf(-6.0f,x[i])); y[i]=0.5f*v;
  }
}
int main(int argc, char **argv) {
  ...
  for (int i=0;i<n;++i) { hx[i] = 8.0f * hs(i, 123); hg[i] = 1.0f * hs(i, 321); }
  ...
  int tpb=256, grid=(n+tpb-1)/tpb; vllmLogitsClampScale2<<<grid,tpb>>>(dx,dg,dy,n);
