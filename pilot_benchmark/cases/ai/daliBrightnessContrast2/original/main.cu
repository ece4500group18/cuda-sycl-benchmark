...
__global__ void image_kernel(const float*x,float*y,int N,int H,int W,int C){int idx=blockIdx.x*blockDim.x+threadIdx.x,total=N*H*W*C;if(idx<total){float v=1.2f*x[idx]+0.05f; y[idx]=fminf(1.0f,fmaxf(0.0f,v));}}
